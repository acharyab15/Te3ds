...
function addShape(type,isNext){
	...
	for (var i=0; i<shapeSize;i++){
		if(isNext){
			nextCubesPoints = nextCubesPoints.concat(TETRIS_CUBE.vertices);
			nextCubesNormals = nextCubesNormals.concat(TETRIS_CUBE.normals);
		}else{
	...
	for (var i=0; i<shapeSize;i++){
		if(isNext){
			...
			shape.cubes[i].ctm = mult(shape.cubes[i].ctm,translate(12.0,-12.0,0.0)) 

			console.log(canvas.height)
			nextTable.push(shape.cubes[i]);
		}else{
