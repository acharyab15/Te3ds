...
//next block CAMERA PARAMETERS******************************//
var eye_next;
var at_next = vec3(0.0,0.0,0.0);
var up_next = vec3(0.0, 0.0, 1.0);
var fovy_next = 45.0; 
var zNear_next = 0.5;
var zFar_next = 40.0;
...
var modelView_next, projection_next;
...
window.onload = function init() {
	...
	eye_next = vec3(0.01,10,40.0)
	modelView = lookAt( eye, at, up );
	modelView_next = lookAt( eye_next, at_next, up_next );
	modelViewInit = lookAt( eye, at, up );
	defaultModelView = lookAt(eye, at, up);
	// Initialize the model view matrix 

	projection = perspective(fovy,aspect,zNear,zFar);
	projection_next = perspective(fovy_next,aspect,zNear_next,zFar_next);
